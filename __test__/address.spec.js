...
describe('test address route', () => {
    test('test address POST method success answer', async () => {
        ...
        expect(JSON.stringify(response)).toBe(JSON.stringify(address));
    });

    test('test address GET ALL method success answer', async () => {
        const userRouter = new UserRouter(pool);

        const res = await request(app.use('/api/users', userRouter.router))
            .get('/api/users/1/address');
    ...
    test('test address GET method success answer', async () => {
        const userRouter = new UserRouter(pool);

        const res = await request(app.use('/api/users', userRouter.router))
            .get('/api/users/1/address/1');
    ...
    test('test address PUT method success answer', async () => {
        const userRouter = new UserRouter(pool);

        const res = await request(app.use('/api/users', userRouter.router))
            .put('/api/users/1/address/1')
            .send({
                address_name: "work",
                address: "3425 Stone Street, Apt. 2A, Jacksonville, FL 39404"
            });
    ...
    test('test address DELETE method success answer', async () => {
        const userRouter = new UserRouter(pool);

        const res = await request(app.use('/api/users', userRouter.router))
            .delete('/api/users/1/address/1');
